...
Public Class Results
    ...
    Public Sub LoadDatainGrd()
        ...
        con.Close()
        cmd = New SqlCommand("Select * from tbl_result with (nolock) where course = '" & cmbCourse.Text & "' and subject = '" & cmbSubject.Text & "' and exam_no = '" & cmbExamno.Text & "'", con)
        con.Open()
        ...
        While dr.Read
            i += 1
            DataGridView1.Rows.Add(dr.Item("RegNo").ToString, dr.Item("Course").ToString, dr.Item("Subject").ToString, dr.Item("Exam_no").ToString, dr.Item("Total_qstn").ToString, dr.Item("correct_qstn").ToString, dr.Item("perc").ToString, dr.Item("status").ToString)
    ...
    Private Sub DataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        If colName = "Column9" Then
            If (MsgBox("Are you sure want to delete record?", vbYesNo + vbQuestion) = vbYes) Then
                qry = "Delete from tbl_result where RegNo =" & DataGridView1.Rows(e.RowIndex).Cells(0).Value.ToString & " and course = '" & cmbCourse.Text & "' and subject = '" & cmbSubject.Text & "' and exam_no = '" & cmbExamno.Text & "'"
                i = InsertData(qry)
                If (i > 0) Then
